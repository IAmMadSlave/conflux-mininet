...
class outChannel {
 ...
 public: /* standard public interface */
  ...
  /** Added for Cesar: if provided with a timeout (in number of
      seconds), the function may return when the function timed out,
      in which case, the retuned value could very well be NULL. */
  Event* getRealEvent(double& rdly, double timeout);
#endif /*PRIME_SSF_EMULATION*/
