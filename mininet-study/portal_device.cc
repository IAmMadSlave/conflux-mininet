...
//					if(mac) {//zzz
					...
					if(1) {
						...
						LOG_DEBUG("##Writer thread sending the evt to sendPacket"<<endl);
						portal_it->second->sendPacket(evt,*mac);
						LOG_DEBUG("##Writer thread Finished Writing"<<endl);
						// the event needs to be reclaimed
						evt->free();
					}
					else {
...
void Portal::init() {
	...
	tx_fd_ip = socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
	if(tx_fd_ip < 0)
	{
		/* If something wrong, just exit */
		LOG_ERROR("Unable to open (tx_fd_ip) raw socket for "<<nic<<"!"<<endl);
	}
...
//		DestAddr.sin_addr.s_addr = inet_addr(tgt);
		...
		if( sendto(tx_fd_ip, temp_buf, my_size, 0, (struct sockaddr *)&DestAddr, sizeof(DestAddr))<0)
		{
