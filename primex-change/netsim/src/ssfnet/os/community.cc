...
void Community::init() {
	...
	cout << "Community "<<getCommunityId()<<" has "<<monitors.size()<<" monitors"<<endl;
...
#if 0
bool Community::getFlow(IPAddress src, IPAddress dst){
	bool exist = false;
	IP2IPMap::iterator ui = symbioFlow.find(src);
	if(ui != symbioFlow.end() && (*ui).second == dst) exist = true;
	ui = symbioFlow.find(dst);
	if(ui != symbioFlow.end() && (*ui).second == src) exist = true;
	return exist;
}

void Community::setSymbioTraffic(SSFNET_STRING traffic_file, SSFNET_STRING name_ip_file){
//		std::cout << "COMMUNITY DEBUG: check name_ip file: "<< name_ip_file << endl;
		ifstream ipfile(name_ip_file);
		SSFNET_MAP(SSFNET_STRING, IPAddress) nameToIp;
				string line;
				if (ipfile.is_open()){
					while (ipfile.good()){
...
//						std::cout << "COMMUNITY DEBUG: UName is: " << myuname << " IP: " << myip << endl;
						nameToIp.insert(SSFNET_MAKE_PAIR(myuname, myip));
						}
					}
					ipfile.close();
				}
				else {std::cout << "COMMUNITY ERROR: unable to open file: " << name_ip_file << endl;}
//		std::cout << "COMMUNITY DEBUG: check traffic file: "<< traffic_file << endl;
		ifstream trafficfile(traffic_file);
		if (trafficfile.is_open()){
			while (trafficfile.good()){
				SSFNET_VECTOR(string) elems;
				...
				if (!elems.empty()) {
				IPAddress sendip = nameToIp[elems[0].c_str()];
				IPAddress recvip = nameToIp[elems[1].c_str()];
//				std::cout << "COMMUNITY DEBUG: sender ip is: " << sendip << " receiver ip is: " << recvip << endl;
				symbioFlow.insert(SSFNET_MAKE_PAIR(sendip, recvip));
				}
			}
			trafficfile.close();
		}
		else {std::cout << "COMMUNITY ERROR: unable to open file: " << traffic_file << endl;}
}
#endif
...
bool Community::getFlow(IPAddress src, IPAddress dst){
	bool exist = false;
	for(IDIPPairMap::iterator ui = symbio.begin(); ui!= symbio.end(); ui++){
		if((*ui).second.first == src){
			if((*ui).second.second == dst) exist = true;
		}
		else if((*ui).second.first == dst){
					if((*ui).second.second == src) exist = true;
				}
	}
//	string a = (exist)?"TRUE":"FALSE";
//	std::cout << "Community DEBUG: this flow is emulated: "<< a << endl;
	return exist;
}

void Community::setSymbioTraffic(){
	Net* net = this->getPartition()->getTopnet();
	...
	if (number_subnets != 0){
			while(rvv.hasMoreElements()){
				...
				while(rv.hasMoreElements()){
					...
					SSFNET_MAP(int,bool) map = host->getSymbioMap();
					if(!(map.empty())){
			//			std::cout << "Community DEBUG: hosts has emulated flow map: " << host->getUName() << endl;
						IPAddress ip = host->getDefaultIP();
							for(SSFNET_MAP(int,bool)::iterator ei = map.begin(); ei != map.end(); ei++){
								int flowid_ = (*ei).first;
								bool sender_ = (*ei).second;
								if (symbio.find(flowid_) == symbio.end()){
									if(sender_) symbio.insert(SSFNET_MAKE_PAIR(flowid_,SSFNET_MAKE_PAIR(ip,0)));
									else symbio.insert(SSFNET_MAKE_PAIR(flowid_,SSFNET_MAKE_PAIR(0,ip)));
									}
								else {
									if(sender_) symbio[flowid_].first = ip;
									else symbio[flowid_].second = ip;
									}
								}
							}
				}
...
#if RONG_DEBUG
	for(IDIPPairMap::iterator fi = symbio.begin(); fi != symbio.end(); fi++){
	std::cout << "Community DEBUG: flow_id: "<< (*fi).first << "\tsender: "
	<< (*fi).second.first << " receiver: " << (*fi).second.second << endl;
		}
#endif
}
#endif
...
//					std::cout <<"COMMUNITY DEBUG:The start time : "<<((StartTrafficEvent*)evt_)->getStartTime().second()<<endl;
					...
					if(h){
//						std::cout <<"COMMUNITY DEBUG:The start traffic host is: "<<h->getUName()<<", its uid is:"<<h->getUID()<<endl;
//						std::cout <<"COMMUNITY DEBUG: traffic event is:"<<evt_->getTrafficType()<<endl;
