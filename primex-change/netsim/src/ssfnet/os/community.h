...
public:
	...
	typedef SSFNET_PAIR(IPAddress,IPAddress) IPPair;
...
//	typedef SSFNET_MAP(IPAddress, IPAddress) IP2IPMap;
	typedef SSFNET_MAP(int, IPPair) IDIPPairMap;
...
#if SYMBIOTIC
	bool getFlow(IPAddress src, IPAddress dst);
	void setSymbioTraffic();
#endif
//	void setSymbioTraffic(SSFNET_STRING traffic_file, SSFNET_STRING name_ip_file);
...
/** a map of sender-receiver pairs of symbio traffic flows
	IP2IPMap symbioFlow;
**/
#if SYMBIOTIC
	/** a map of sender-receiver pairs of symbio traffic flows **/
