...
//#define FLOW_FILE "/Users/rong/Documents/workspace/primex/netsim/flow_file"
//#define NAME_UID_FILE "/Users/rong/Documents/workspace/primex/netscript/exp/dumbelltrafficxml/dumbelltrafficxml_node_uid.txt"
...
static void show_usage(char* prognam) {
	...
	fprintf(stderr,	"  -routes <file>       : the route specification for routers/hosts\n");
	fprintf(stderr,	"                         with a 'manual' routing protocol.\n");
	fprintf(stderr, "  -R                   : same as -routes.\n");
...
(int argc, char** argv) {
	...
	offset = 1;
	for (int i = 1; i < argc; i++) {
		if (!strcasecmp(argv[i], "-h") || !strcmp(argv[i], "-help")) {
			showuse = true;
			break;
		}
		else if (!strcasecmp(argv[i], "-e") || !strcmp(argv[i], "-emuratio")) {
			if (++i >= argc) {
		...
		else if (!strcmp(argv[i], "-R") || !strcmp(argv[i], "-routes")) {
			if (++i >= argc) {
				fprintf(stderr, "ERROR: -R or -routes: argument missing!\n\n");
				showuse = true;
				break;
			}
			ManualRoutes::parseFile(argv[i]);
		}
	...
	#if SYMBIOTIC
		if (argc != 5) {
		std::cout << "SSFNET DEBUG: wrong symbiotic arguments: " << argc << endl;
		std::cout << "USAGE: %s <sim-time> <model source> <downscale model> <update info>\n";
	#endif
#endif
...
#if SYMBIOTIC
//	std::cout << "SSFNET DEBUG: I am entering symbiosim world!!! "<< endl;
	if(argc == 5){
		std::cout << "SSFNET DEBUG: correct number of input arguments: simulation time: " << end_time
		<< "\n\tmodel_file: " << argv[2]
		<< "\n\tdownscale_model_file: " << argv[3] << "\n\toutput_file: " << argv[4] << endl;}
	else {std::cout << "SSFNET_ERROR: not enough number of input arguments for symbiosim!" << endl;}
#endif

	//call init on all the model entities
	partition->init();
	ManualRoutes::cleanup();
...
//Rong's code
#if SYMBIOTIC
	Community* com = partition->getCommunityWithLowestId();
	com->setSymbioTraffic();
	SimulationSamplingTimer* sample = new SimulationSamplingTimer(com,end_time,argv[3],argv[4]);
	sample->resched(PROBE_INTERVAL);
//	std::cout << "SSFNET_DEBUG: setting up the traffic and timer." << endl;
#endif
//	com->setSymbioTraffic(FLOW_FILE,NAME_UID_FILE);
//Rong's code end
...
//	std::cout<< "SSFNET DEBUG: current time is:"<<current_time<<endl;
