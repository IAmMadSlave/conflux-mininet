...
void Interface::init() {
	...
	unshared.arrival_symbio_packets.write(0);
	unshared.arrival_symbio_bytes.write(0);
}
...
#if SYMBIOTIC
int Interface::getSymbioBytes() {
	return unshared.arrival_symbio_bytes.read();
}
int Interface::getSymbioPackets() {
	return unshared.arrival_symbio_packets.read();
}
#endif
...
#if SYMBIOTIC
	bool symbio_or_not = this->inHost()->getCommunity()->getFlow(ipmsg->getSrc(),ipmsg->getDst());
	if (symbio_or_not){ // collect symbio statistics
			uint x = unshared.arrival_symbio_packets.read();
			unshared.arrival_symbio_packets.write(x + 1);
			x = unshared.arrival_symbio_bytes.read();
			unshared.arrival_symbio_bytes.write(x + pkt->size());
		}
#endif
//	else {std::cout << "INTERFACE DEBUG: this is not a symbio packet!" << endl;}
