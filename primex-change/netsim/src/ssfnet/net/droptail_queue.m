...
public:
	...
	shadow_queue(0), 
	#endif
		bytes_dropped(0), bytes_total(0), queue_file(NULL), flow_file(NULL), loss_file(NULL), avgqueue(0), mean_pkt(4000), weight(0) {
	}
...
#ifdef FLUID_TRAIN	
	void updateTrain(uint64 arrival_rate, uint64 existing_rate, VirtualTime duration, uint64& bytes_delivered, uint64& dropped_bytes, VirtualTime& delay, int& predict_queue);
	int shadow_queue;
#endif
	uint64 bytes_dropped;
	uint64 bytes_total;
    uint64 avgqueue;
    uint64 mean_pkt;
    float weight;
