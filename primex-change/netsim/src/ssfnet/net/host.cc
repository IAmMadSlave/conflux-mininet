...
void Host::init() {
	...
	ProtocolGraph::init();
/*
		for (SSFNET_VECTOR(ProtocolSession*)::iterator iter = unshared.protocol_list.begin();
				iter != unshared.protocol_list.end(); iter++) {
			std::cout << "HOST Debug: session on tap: " << (*iter)->getName() << endl;
		}
*/
	...
	while (sessions.hasMoreElements()) {
		...
		if(RoutingProtocol::getClassConfigType()->isSubtype(s->getConfigType())) {
			if(fwd_engine) {
				...
				std::cout <<" NATE:: found routing forwarding engine! node="<<getUName()<<endl;
...
#if SYMBIOTIC
		else if (Sender::getClassConfigType()->isSubtype(s->getConfigType())){
#if RONG_DEBUG
			std::cout << "HOST DEBUG: found symbiotic sender! node= "<<getUName() << endl;
#endif
			Sender* sender = SSFNET_DYNAMIC_CAST(Sender*,s);
			symbio_id_sender.insert(SSFNET_MAKE_PAIR(sender->getFlowId(),true));

		}
		else if (Receiver::getClassConfigType()->isSubtype(s->getConfigType())){
#if RONG_DEBUG
			std::cout << "HOST DEBUG: found symbiotic receiver! node= "<<getUName()<<endl;
#endif
			Receiver* receiver = SSFNET_DYNAMIC_CAST(Receiver*,s);
			symbio_id_sender.insert(SSFNET_MAKE_PAIR(receiver->getFlowId(),false));
				}
#endif
	}
	if(!fwd_engine) {
...
//	std::cout << "Host DEBUG: current host is: " << this->getName() << endl;
#if RONG_DEBUG
	for(SSFNET_MAP(int,bool)::iterator si = symbio_id_sender.begin(); si!= symbio_id_sender.end(); si++){
		std::cout << "\tsymbio flow id: " << (*si).first << " sender?: " << (*si).second << endl;
	}
#endif
}
...
#if SYMBIOTIC
SSFNET_MAP(int,bool) Host::getSymbioMap(){
	return symbio_id_sender;
}
#endif
...
IPAddress Host::getDefaultIP() {
	if (!unshared.default_nic) {
		ChildIterator<Interface*> si = nics();
		while (si.hasMoreElements()) {
			Interface* iface = si.nextElement();
			if (iface == NULL)
				LOG_ERROR("the iface was null -- should never see this!"<<endl);
			unshared.default_nic = iface;
		}
	}
	if(unshared.default_nic)
		return unshared.default_nic->getIP();
	return IPAddress::IPADDR_INVALID;
}

float Host::getDefaulBitrate() {
	if (getDefaultIP() == IPAddress::IPADDR_INVALID) {
		return 0;
	}
	return unshared.default_nic->getBitRate();
}
...
MACAddress Host::getDefaultMAC() {
	...
	return unshared.default_nic->getMAC();
}
...
void Host::handleStartTrafficEvent(StartTrafficEvent* evt){
	...
	//std::cout<<"the event on the top of start_traffics is:"<<evt<<endl;
	//std::cout<<"The start time is: "<<evt->getStartTime().second()<<", now="<<VirtualTime(prime::ssf::now())<<",oldmin="<<old_min.second() <<endl;
