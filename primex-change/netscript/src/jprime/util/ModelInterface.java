...
public abstract class ModelInterface {
	...
	public final PartTlvPair createTLV(
		...
		exp.save();
		ArrayList<IHost> hosts_list=exp.getHostsList();
		ArrayList<IRouter> routers_list=exp.getRoutersList();
			if(hosts_list!=null&&!hosts_list.isEmpty()){
		        //write host uids to a file
				String file= "/Users/rong/Documents/workspace/primex/netscript/exp/" + exp.getName().toString() + "/" + exp.getName().toString() + "_node_uid.txt";
//				System.out.println(file);
		        FileWriter fstream = new FileWriter(file);
		        FileWriter ostream = new FileWriter("/Users/rong/Documents/workspace/primex/netscript/exp/" + exp.getName().toString() + "/" + exp.getName().toString() + "_interface_uid.txt");
		        BufferedWriter out = new BufferedWriter(fstream);
		        BufferedWriter out2 = new BufferedWriter(ostream);
				for(int i=0; i<hosts_list.size();i++){
					IHost m=hosts_list.get(i);
					Long uid=m.getUID();
					String ip="";
					for(ModelNode c : m.getAllChildren()) {
						if(c instanceof IInterface) {
							ip=((IInterface)c).getIpAddress().getValue();
							Long iid = ((IInterface)c).getUID();
							String name = ((IInterface)c).getUniqueName().toString();
							String line2 = Long.toString(iid)+","+ip+","+name+"\n";
							out2.write(line2);
//							System.out.println(line2);
						}
					}
					String line = Long.toString(uid)+","+ip+","+m.getUniqueName().toString()+"\n";
			...
			else{System.out.println("No hosts in hosts_list!");}
			
			if(routers_list!=null&&!routers_list.isEmpty()){
		        //write host uids to a file
		        FileWriter ostream = new FileWriter("/Users/rong/Documents/workspace/primex/netscript/exp/"+ exp.getName().toString() + "/" + exp.getName().toString() + "_interface_uid.txt",true);
		        BufferedWriter out2 = new BufferedWriter(ostream);
				for(int i=0; i<routers_list.size();i++){
					IRouter m=routers_list.get(i);
					for(ModelNode c : m.getAllChildren()) {
						if(c instanceof IInterface) {
							String ip=((IInterface)c).getIpAddress().getValue();
							Long iid = ((IInterface)c).getUID();
							String name = ((IInterface)c).getUniqueName().toString();
							String line2 = Long.toString(iid)+","+ip+","+name+"\n";
							out2.write(line2);
//							System.out.println(line2);
			...
			else{System.out.println("No routers in routers_list!");}
